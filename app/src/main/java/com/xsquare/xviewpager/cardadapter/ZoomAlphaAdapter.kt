...
class ZoomAlphaAdapter(var lists:ArrayList<CardBean>) : BasePagerAdapter(lists) {

    override fun initItem(container: ViewGroup?, position: Int): View {
        val bean = lists[position]
        val view = LayoutInflater.from(container!!.context).inflate(R.layout.page_adapter_zoom_alpha,container,false)
